...
2023-08-28 Update: incorporate non-specific antibody binding function in simulation
"""
...
class simulatePLA:
    ...
    def simulate(self, num_complex, probeA, probeB, non_specific_prob_A, non_specific_prob_B, verbose=True):
        '''
        ...
        non_specific_prob_A & non_specific_prob_B: list or numpy array
            An N-by-1 array containing the probability of non-specific binding for each antibody
        ...
        non_specific_prob_A = np.array(non_specific_prob_A)
        non_specific_prob_B = np.array(non_specific_prob_B)
        ...
        self.non_specific_prob_A = non_specific_prob_A.copy()
        self.non_specific_prob_B = non_specific_prob_B.copy()
        ...
        # Dictionary to store non-specific probe abundance of each single cell
        dge_probe_ns = {}
        ...
        variance_probeB_ns = []
        variance_complex = []
        variance_non_specific_probeA_ns = []
        variance_non_specific_probeB_ns = []
        if self.protein_variance:
            for i in range(len(self.probeA)):
                ...
                variance_non_specific_probeA_ns.append(temp_scale*self.non_specific_prob_A[i])
                variance_non_specific_probeB_ns.append(temp_scale*self.probeB[i]/self.probeA[i]*self.non_specific_prob_B[i])
            ...
            variance_non_specific_probeA_ns = np.array(variance_non_specific_probeA_ns)
            variance_non_specific_probeB_ns = np.array(variance_non_specific_probeB_ns)
        ...
        for cell_i in range(self.n_cells):
            ...
            dge[cell_i].update({f'{i+1}_nsA{self.sep}{j+1}':0 for i in range(len(self.probeA)) for j in range(len(self.probeB))})
            dge[cell_i].update({f'{i+1}_nsA{self.sep}{j+1}_nsB':0 for i in range(len(self.probeA)) for j in range(len(self.probeB))})
            dge[cell_i].update({f'{i+1}{self.sep}{j+1}_nsB':0 for i in range(len(self.probeA)) for j in range(len(self.probeB))})

            # Add protein variance
            if self.protein_variance:
                probeA_i = copy.deepcopy(variance_probeA_ns[:,cell_i]).astype(int)
                non_specific_probeA_i = copy.deepcopy(variance_non_specific_probeA_ns[:,cell_i]).astype(int)
                probeB_i = copy.deepcopy(variance_probeB_ns[:,cell_i]).astype(int)
                non_specific_probeB_i = copy.deepcopy(variance_non_specific_probeB_ns[:,cell_i]).astype(int)
                num_complex_i = copy.deepcopy(variance_complex[:,:,cell_i]).astype(int)
            else:
                probeA_i = copy.deepcopy(probeA).astype(int)
                non_specific_probeA_i = copy.deepcopy(probeA*non_specific_prob_A).astype(int)
                probeB_i = copy.deepcopy(probeB).astype(int)
                non_specific_probeB_i = copy.deepcopy(probeB*non_specific_prob_B).astype(int)
            ...
            #Save the non_specific antibody abundance
            dge_probe_ns[cell_i] = np.hstack((non_specific_probeA_i, non_specific_probeB_i))
            ...
            # Generate non_specific probes A and B
            if non_specific_probeA_i.sum() > 0:
                if self.mode == '2D':
                    temp_non_specific_probeA_i = self.cell_d/2*randomPointGen2D(non_specific_probeA_i.sum())
                elif self.mode == '3D':
                    temp_non_specific_probeA_i = self.cell_d/2*randomPointGen3D(non_specific_probeA_i.sum())
            # Target id of each non-specific probe A molecule
            temp_non_specific_probeA_i_targets = np.repeat([f"{s+1}_nsA" for s in range(len(non_specific_probeA_i))], non_specific_probeA_i)
            
            if non_specific_probeB_i.sum() > 0:
                if self.mode == '2D':
                    temp_non_specific_probeB_i = self.cell_d/2*randomPointGen2D(non_specific_probeB_i.sum())
                elif self.mode == '3D':
                    temp_non_specific_probeB_i = self.cell_d/2*randomPointGen3D(non_specific_probeB_i.sum())
            # Target id of each non-specific probe B molecule
            temp_non_specific_probeB_i_targets = np.repeat([f"{s+1}_nsB" for s in range(len(non_specific_probeB_i))], non_specific_probeB_i)
            ...
            # x,y,z coordinates
            if non_specific_probeA_i.sum() > 0:
                probeA_i = np.vstack((temp_probeA_i, temp_non_specific_probeA_i, copy.deepcopy(temp_complex_i)))
                # Probe target name
                probeA_targets = np.concatenate((temp_probeA_i_targets,  temp_non_specific_probeA_i_targets, complex_probeA_targets))
            else:
            ...
            if non_specific_probeB_i.sum() > 0:
                probeB_i = np.vstack((temp_probeB_i, temp_non_specific_probeB_i, copy.deepcopy(temp_complex_i)))
                # Probe target name
                probeB_targets = np.concatenate((temp_probeB_i_targets, temp_non_specific_probeB_i_targets, complex_probeB_targets))
            else:
        ...
        temp_pla_count = pd.DataFrame(dge)
        ...
        index_specific = [f'{i+1}{self.sep}{j+1}' for i in range(num_complex.shape[0]) for j in range(num_complex.shape[1])]
        
        index_non_specific = [f'{i+1}_nsA{self.sep}{j+1}_nsB' for i in range(num_complex.shape[0]) for j in range(num_complex.shape[1])]
        
        index_A_non_specific = [f'{i+1}_nsA{self.sep}{j+1}' for i in range(num_complex.shape[0]) for j in range(num_complex.shape[1])]             
        index_B_non_specific = [f'{i+1}{self.sep}{j+1}_nsB' for i in range(num_complex.shape[0]) for j in range(num_complex.shape[1])]
        
        self.pla_specific = temp_pla_count.loc[index_specific,:]
        
        self.pla_non_specific = temp_pla_count.loc[index_non_specific,:]
        
        self.pla_one_specific = pd.concat([temp_pla_count.loc[index_A_non_specific,:], temp_pla_count.loc[index_B_non_specific,:]])
        
        temp_pla_count.index = temp_pla_count.index.str.replace('_nsA', '').str.replace('_nsB', '')
        
        self.pla_count = temp_pla_count.groupby(temp_pla_count.index).sum()
        ...
        self.probe_ns = pd.DataFrame(dge_probe_ns, index=[f"{i+1}_A"for i in range(num_complex.shape[0])] +
                                        [f"{i+1}_B" for i in range(num_complex.shape[1])])
